  ...
  return (
    <button className={styles.themeBtn} onClick={toggleTheme} title={`Switch theme`}>
      <div className={`${styles.themeTrack} ${isAfrican ? styles.themeTrackOn : ''}`}>
        <div className={styles.themeThumb}>{isAfrican ? '🌿' : '🌙'}</div>
      </div>
      <span className={styles.themeBtnLabel}>{isAfrican ? 'East African' : 'Arabian'}</span>
    </button>
  )
}
...
export default function Home({ theme, toggleTheme }) {
  ...
  return (
    <Layout theme={theme} toggleTheme={toggleTheme}>
      <Head><title>Read · Tarteel</title></Head>
